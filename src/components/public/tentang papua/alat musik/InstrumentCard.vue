...
const emit = defineEmits<{
  (e: "open-detail", instrument: Instrument): void;
  (e: "toggle-favorite", id: string): void;
}>();

const handleClick = () => {
  emit("open-detail", props.instrument);
};

const handleFavorite = (event: MouseEvent) => {
  event.stopPropagation();
  emit("toggle-favorite", props.instrument.id);
};
</script>
...
<template>
  ...
  >
    ...
    >
      ...
      >
        <span v-if="instrument.isFavorite">★</span>
        <span v-else>☆</span>
      </button>
    </div>
