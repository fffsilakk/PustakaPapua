...
const emit = defineEmits<{
  (e: "open-detail", senjata: Senjata): void;
  (e: "toggle-favorite", id: string): void;
}>();

const handleClick = () => {
  emit("open-detail", props.senjata);
};

const handleFavorite = (event: MouseEvent) => {
  event.stopPropagation();
  emit("toggle-favorite", props.senjata.id);
};
</script>
...
<template>
  ...
  >
    ...
    >
      ...
      >
        <span v-if="senjata.isFavorite">★</span>
        <span v-else>☆</span>
      </button>
    </div>
    ...
    <div class="flex flex-1 flex-col px-4 pb-4 pt-3">
      ...
      >
        <div class="flex flex-wrap gap-1.5">
          ...
          >
            🗡️ {{ senjata.type.replace("-", " ") }}
          </span>
          <span
            v-if="senjata.usage?.length"
